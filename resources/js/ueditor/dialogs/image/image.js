...
(function() {
    ...
    function insertSingle() {
        ...
        var url = g("url"),
                ...
                activityType = g('activityType').value,
                imgObj = {};
        if (!url.value)
            return;
        if (!flagImg)
        ...
        if (link) {
            if (activityType == 'goods') {
                imgObj.onclick = "redirectToShopDetail(\'" + link + "\')";
            } else if (activityType == 'con') {
                imgObj.onclick = 'MYAPP.gopage(\'contentDetail\',\'' + link + '\')';
            } else if (/^(myapp):\/\/(.*)+/i.test(link) || /^(MYAPP.gopage)(.*)+/i.test(link)) {
                imgObj.onclick = link;
            } 
        }
        insertImage(imgObj);
    ...
    function insertBatch() {
        var imageUrl = $("#flashContainer img");
        ...
        imageUrl.each(function(i) {
            ...
            tmpObj.title = '图片';
            tmpObj.floatStyle = '';
            tmpObj.data_ue_src = tmpObj.src = $(this).attr("src");
            imgObjs.push(tmpObj);
        })
