...
function Calendar(element, instanceOptions) {
	...
	function updateTitle() {
		header.updateTitle(currentView.newYear);
		header.updateTitleM(currentView.newMonth);
	}
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h2>&nbsp;</h2><p>&nbsp;</p>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
...
$.extend(MonthView.prototype, {
	...
	render: function(date) {
        ...
        var arrdata=this.title.split(" ");
        this.newYear=arrdata[1];
        this.newMonth=arrdata[0];
